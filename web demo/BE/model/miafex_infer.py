...
if __name__ == "__main__":
    ...
    train_set = chestCTforViT(
        config["data"]["path"], "train", config["data"]["img_size"]
    )
    ...
    train_loader = DataLoader(
        train_set, batch_size=config["data"]["batch_size"], shuffle=True
    )
    dev_loader = DataLoader(
        dev_set, batch_size=config["data"]["batch_size"], shuffle=False
    )
    ...
    optimizer = optim.NAdam(
        filter(lambda p: p.requires_grad, model.parameters()), lr=config["adam"]["lr"]
    )
    ...
    for model in list_of_models:
        ...
        model.save(config["svm"]["model_path"])
